...
// TBF burst to 1 MB in the node root netns. Catches the host-side
// IFB qdiscs the bandwidth plugin installs for ingress shaping,
// which is what the lima rig needs (and where the measurement on
// lima caps cleanly at ~10 Mbps).
//
// Known limitation: on k3d the iperf-egress direction (and to a
// lesser extent the ingress mixed-workload direction) overshoots
// the annotated cap by 2-10× because kubelet's huge default TBF
// burst (~150s of credit, ~193 MB on a 10 Mbps annotation) sits
// on the pod-side egress TBF, which lives in the pod netns and
// isn't reachable from the node root netns. An earlier
// nsenter-into-pod-netns "pass 2" extension was tried and
// rejected — it made vanilla iperf3 worse on k3d (99 Mbps vs the
// 23 Mbps overshoot pass 1 alone produces), the escaping inside
// nsenter sh -c '<script>' was fragile, and the cost wasn't worth
// the complexity. Documented in perf-vs-vanilla.md as a k3d
// limitation.
//
// The rate field is preserved on every change (different
// annotations map to different rates; only the burst needs
// squashing). Idempotent.
...
// so callers must pass --runtime-endpoint explicitly. Used by the
// memory workload's installer-DS-RSS reader.
